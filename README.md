...
| `ProcessedAt` | `DateTime?` | Дата и время обработки брони (заполняется при смене статуса) |

### Статусы бронирования (`BookingStatus`)

| Статус | Описание |
|--------|----------|
| `Pending` | Бронь создана, ожидает обработки. Начальный статус. |
| `Confirmed` | Бронь подтверждена. Устанавливается фоновым сервисом после обработки. |
| `Rejected` | Бронь отклонена. (Будет использоваться в следующих спринтах.) |

> **ℹ️ Важно**:  
> Бронирование создаётся со статусом `Pending`. Фоновый сервис `BookingBackgroundService` периодически обрабатывает `Pending`-заявки, имитирует внешний вызов (задержка 2 секунды) и переводит их в `Confirmed`. После обработки заполняется поле `ProcessedAt`.
